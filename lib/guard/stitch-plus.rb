...
class Stitcher
  ...
  def compile
    ...
    if @files.join().match(/\.coffee/) and !@has_coffee
      Guard::UI.error "Cannot compile coffeescript".red
      Guard::UI.error "Add ".white + "gem 'coffee-script'".yellow + " to your Gemfile."
    end

    if @options[:uglify] and !@uglifier
      Guard::UI.error "Cannot uglify javascript".red
      Guard::UI.error "Add ".white + "gem 'uglifier'".yellow + " to your Gemfile."
    end

    @fingerprint = script_fingerprint(@files)
    @file = output_file(@fingerprint)

    if has_fingerprint(@file)
      Guard::UI.info "Stitch " + "identical ".green + @file
      true
    else
      begin
        ...
        Guard::UI.info "Stitch " + write_msg
        true
      rescue StandardError => e
        Guard::UI.error "Stitch " + "failed ".red + "to write #{@file}"
        Guard::UI.error e
        false
      end
    end
  ...
  def cleanup(file)
    ...
    Dir.glob(File.join(File.dirname(@options[:output]), '**/*')).each do |item|
      if File.basename(item) != File.basename(file) and File.basename(item).match /^#{match[0]}(-.+)?#{match[1]}/i
        Guard::UI.info "Stitch " + "deleted ".red + item
        FileUtils.rm(item) 
      end
    end
  end
